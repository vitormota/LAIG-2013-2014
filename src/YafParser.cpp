...
void test(TiXmlElement *element) {
    char *root = (char*) element->Attribute("rootid");


    TiXmlElement *node = element->FirstChildElement();
    while (node) {
	char *nodeid = (char*) node->Attribute("id");

	TiXmlElement *transf = node->FirstChildElement("transforms");
	if (transf) {

	    transf = transf->FirstChildElement();
	    while (transf) {
		//transforms
		char *value = (char*) transf->FirstAttribute()->Value();
		char *value1 = (char*) transf->Attribute("to");
		char *value2 = (char*) transf->Value();
...
namespace Parser {
	...
	//Test
	test(yafElement->FirstChildElement("graph"));
	//end Test
	...
	while (nodeElement) {
		...
		char *attr=NULL;
		attr = (char*) transformationElement->Attribute("to");
		if(attr){
		    float to[3];
		    if(StringParsing::FloatReader(attr,to)==3){
			//valid "to" attribute
			glTranslatef(*to,*(to+1),*(to+2));
		    }else{
			//invalid
		    }
		}
		attr = (char*) transformationElement->Attribute("factor");
		if(attr){
		    float factor[3];
		    if(StringParsing::FloatReader(attr,factor)==3){
			//valid "factor" attribute
			glScalef(*factor,*(factor+1),*(factor+2));
		    }else{
			//invalid
		    }
		}
		attr = (char*) transformationElement->Attribute("axis");
		if(attr){
		    float angle=0;
		    transformationElement->QueryFloatAttribute("angle",&angle);
		    char axis = attr[0];
		    if(angle){
			//valid "rotate" element
			switch(axis){
			    case 'x':
