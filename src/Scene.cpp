...
void Scene::processGraph() {
    ...
    glPushMatrix();
    processNode(rootid, false);
    glPopMatrix();
}

void Scene::processNode(string id, bool onList) {
    ...
    if (currentNode == NULL) {
        ...
        if (!display_list || (display_list && currentNode->the_object == 0x0)) {
            ...
            if (display_list && onList == false) {
                //parent not on List but this one is so
                onList = true;
                //initialize list
                created = true;
        ...
        //next list only if node is the creator parent
        if(created) current_list_index++;
        //----> END NEW
